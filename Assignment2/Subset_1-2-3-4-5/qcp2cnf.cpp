...
// Every number in [n] appears in every row;
vector<vector<int>> property_e()
{

    vector<vector<int>> property_e_clauses;
    ...
    for (int i = 1; i <= order; i++)
    {
        ...
        for (int k = 1; k <= order; k++)
        {
            ...
            property_e_clauses.push_back(clauses);
        }
    }

    return property_e_clauses;
}
...
int main(int argc, char *argv[])
{
    ...
    // property d clauses
    vector<vector<int>> property_e_clauses = property_e();
    ...
    // lines of clauses
    int lines = unit_clauses.size() + property_a_clauses.size() + property_b_clauses.size() + +property_c_clauses.size() + property_d_clauses.size() + property_e_clauses.size();
    ...
    // property e clauses
    for (int i = 0; i < property_e_clauses.size(); i++)
    {
        for (int j = 0; j < property_e_clauses[i].size(); j++)
        {
            output_file << property_e_clauses[i][j] << " ";
        }
        output_file << "0\n";
    }
